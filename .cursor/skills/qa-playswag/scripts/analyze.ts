#!/usr/bin/env npx tsx
/**
 * playswag — API Coverage Analyzer (TypeScript)
 * Analyzes OpenAPI/Swagger spec vs existing test files
 * Generates HTML report + Markdown tasks for QA automation engineers
 *
 * Usage:
 *   npx tsx analyze.ts <spec-file> [tests-dir]
 *   npx tsx analyze.ts spec1.yaml spec2.yaml -- tests/
 *   npx tsx analyze.ts https://api.example.com/openapi.json tests/
 */
...
interface StatusCodeAssertion {
  code: string;
  file: string;
  line: number;
}

interface TagCoverageData {
  total: number;
  covered: number;
  pct: number;
}
...
interface CoverageReport {
  specFile: string | string[];
  testsDir: string;
  timestamp: string;
  endpoints: OAEndpoint[];
  testHits: TestHit[];
  covered: OAEndpoint[];
  uncovered: OAEndpoint[];
  deprecated: OAEndpoint[];
  deprecatedInTests: Array<{ endpoint: OAEndpoint; hits: TestHit[] }>;
  orphanHits: TestHit[];
  endpointCoverage: number;
  statusCodeCoverage: number;
  parameterCoverage: number;
  coverageByTag: Record<string, TagCoverageData>;
}
...
function fetchUrl(url: string): string {
  ...
  for (const [cmd, args] of commands) {
    try { execFileSync(cmd, args); return cached; } catch {}
  }
...
// ─── $ref resolver — keep in sync with analyze.js / analyze.py ────────────────

function getPointerParts(refHash: string): string[] {
  return refHash.slice(2).split("/").map((p) => p.replace(/~1/g, "/").replace(/~0/g, "~"));
}

function getAtPointer(root: any, parts: string[]): any {
  let cur: any = root;
...
function resolveRefs(spec: any, maxDepth = 10): any {
  const root = spec;
  const resolving = new Set<string>();

  function resolve(obj: any, depth: number): any {
    ...
    if (typeof obj.$ref === "string" && obj.$ref.startsWith("#/")) {
      ...
      for (const [k, v] of Object.entries(obj)) {
        if (k !== "$ref") (merged as Record<string, unknown>)[k] = resolve(v, depth + 1);
      }
      resolving.delete(ref);
      return merged;
    }
...
function joinBaseAndPath(base: string, tpl: string): string {
...
function extractPathFromServerUrl(url: string): string {
...
function extractBasePaths(spec: any): string[] {
  const out: string[] = [];
...
function hitMatchesEndpointPath(hitPath: string, templatePath: string, basePaths: string[]): boolean {
...
function extractBodyFields(schema: any, prefix = "", depth = 3): string[] {
  if (!schema || depth === 0) return [];
  const fields: string[] = [];
  let props: Record<string, any> = schema.properties || {};

  for (const key of ["allOf", "anyOf", "oneOf"] as const) {
    for (const sub of (schema[key] || []) as any[]) {
...
function parseSpec(spec: any): OAEndpoint[] {
  const endpoints: OAEndpoint[] = [];
  const isOAS3 = !!spec.openapi;
  const paths = spec.paths || {};
  const globalSecurity = spec.security || [];

  for (const [pathTemplate, pathItem] of Object.entries(paths as Record<string, any>)) {
    ...
    for (const method of HTTP_METHODS) {
      ...
      if (isOAS3 && op.requestBody) {
        ...
        bodyFields = extractBodyFields((jsonContent as any).schema || {}, "", 3);
      } else if (!isOAS3) {
        const bodyParam = (op.parameters || []).find((p: any) => p && p.in === "body");
...
function filterEndpoints(
  endpoints: OAEndpoint[],
  opts: Pick<CliOpts, "include" | "exclude" | "includeTags" | "excludeTags">
): { endpoints: OAEndpoint[]; ignored: OAEndpoint[] } {
  let filtered = endpoints;
  const ignored: OAEndpoint[] = [];

  if (opts.include && opts.include.length) {
    const next: OAEndpoint[] = [];
...
const HIT_PATTERNS: Array<{ re: RegExp; mGrp: number; uGrp: number }> = [
...
function scanTestFile(filePath: string): TestHit[] {
  let content: string;
...
const STATUS_ASSERTION_PATTERNS: RegExp[] = [
  /\.expect\s*\(\s*(\d{3})\s*\)/g,
...
function scanStatusCodeAssertions(filePath: string): StatusCodeAssertion[] {
  let content: string;
  try { content = fs.readFileSync(filePath, "utf8"); } catch { return []; }

  const results: StatusCodeAssertion[] = [];
  const seen = new Set<string>();
...
function scanParameterUsage(filePath: string, paramNames: string[]): Set<string> {
...
function pathMatchesTemplate(hitPath: string, template: string): boolean {
...
function pathPrefixMatchesTemplate(hitPrefix: string, template: string): boolean {
...
function matchHitToEndpoint(hit: TestHit, endpoints: OAEndpoint[], basePaths: string[] = []): OAEndpoint | null {
  let best: OAEndpoint | null = null;
  let bestScore = -1;
...
interface CoverageResult {
  ...
  statusCodeCoverage: number;
  parameterCoverage: number;
  perEndpointSC: Map<string, { spec: string[]; tested: string[] }>;
  perEndpointParams: Map<string, { total: string[]; used: string[] }>;
  coverageByTag: Record<string, TagCoverageData>;
}
...
function calculateCoverage(
  ...
  const endpointHitsMap = new Map<string, TestHit[]>();
  const orphanHits: TestHit[] = [];
  ...
  const allStatusAssertions: StatusCodeAssertion[] = [];
  ...
  const perEndpointSC = new Map<string, { spec: string[]; tested: string[] }>();
  for (const ep of endpoints) {
    const key = `${ep.method}:${ep.path}`;
  ...
  const perEndpointParams = new Map<string, { total: string[]; used: string[] }>();
  ...
  const tagMap = new Map<string, { total: number; covered: number }>();
  for (const ep of endpoints) {
  ...
  const coverageByTag: Record<string, TagCoverageData> = {};
  ...
  return {
    covered, uncovered, deprecatedInTests, orphanHits,
...
const methodColor = (m: string): string => METHOD_COLORS[m] || "#999";
const coverageColor = (p: number): string => p >= 80 ? "#49cc90" : p >= 50 ? "#fca130" : "#f93e3e";
const priority = (ep: OAEndpoint): string => ep.deprecated ? "Low" : ["POST", "PUT", "DELETE", "PATCH"].includes(ep.method) || ep.authRequired ? "High" : "Medium";
const priorityColor = (p: string): string => ({ High: "#f93e3e", Medium: "#fca130", Low: "#aaa" } as Record<string, string>)[p] || "#aaa";
const rel = (p: string): string => { try { return path.relative(process.cwd(), p); } catch { return p; } };
...
function generateTasks(report: CoverageReport): string {
...
function generateJunit(report: CoverageReport): string {
...
<testsuite name="PlaySwag API Coverage" tests="${total}" failures="${failures}" errors="0" time="0" timestamp="${report.timestamp}">
${cases.join("\n")}
</testsuite>`;
}
...
function generateBadge(pct: number): string {
...
function loadHistory(outDir: string): HistoryEntry[] {
  const p = path.join(outDir, "playswag-history.json");
  try { return JSON.parse(fs.readFileSync(p, "utf8")); } catch { return []; }
}

function saveHistory(outDir: string, entry: HistoryEntry, maxEntries = 50): HistoryEntry[] {
...
function findTestsDir(specFile: string): string {
...
function parseArgs(argv: string[]): CliOpts {
  ...
  let foundSep = false;
  const beforeSep: string[] = [];
  const afterSep: string[] = [];
...
function main(): void {
  ...
  if (!opts.specFiles.length) {
    console.error(
      "Usage: npx tsx analyze.ts <spec-file> [tests-dir] [options]\n" +
      "       npx tsx analyze.ts spec1.yaml spec2.yaml -- tests/ [options]\n\n" +
      "Options:\n" +
  ...
  let allEndpoints: OAEndpoint[] = [];
  let allBasePaths: string[] = [];
  ...
  for (const sf of opts.specFiles) {
    ...
    let specData: any;
    try { specData = loadSpec(specFile); } catch (e: any) { console.error(`ERROR: ${e.message}`); process.exit(2); }
